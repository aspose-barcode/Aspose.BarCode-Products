...
{{% blocks/products/pf/agp/content h2="How to Generate QR Код Using .NET" %}}
Чтобы создать QR Код, попробуйте [Aspose.BarCode for .NET](https://products.aspose.com/barcode/net/) API, многофункциональную, мощную и простую в использовании библиотеку штрих-кодов. для платформ С#. Откройте диспетчер пакетов [NuGet](https://www.nuget.org/packages/aspose.barcode), найдите **Aspose.BarCode** и установите пакет. Вы также можете запустить следующую команду из консоли диспетчера пакетов:
...
{{% blocks/products/pf/agp/feature-section-col title="Действия по созданию QR Код в C#" %}}
...
<ul><li>Создайте экземпляр класса BarcodeGenerator</li><li>Передайте Код QR, код GS1 QR в качестве параметра</li><li>Установите текст для кодирования в качестве второго параметра</li><li> При необходимости укажите размеры штрих-кода</li><li>Вызовите метод BarcodeGenerator.Save, чтобы получить изображение Код QR, код GS1 QR</li></ul>
