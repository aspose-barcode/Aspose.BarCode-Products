...
{{% blocks/products/pf/agp/content h2="How to Read 인터리브 2-of-5(ITF) 바코드 Using Android via Java" %}}
Java를 통해 Android용 Aspose.BarCode를 사용하여 Android 애플리케이션 내에서 인터리브 2-of-5(ITF) 바코드을 쉽게 감지하고 읽으십시오. Android 애플리케이션을 위한 풍부하고 확장 가능하며 사용자 친화적인 바코드 API입니다. build.gradle에 다음 구성을 추가하여 Maven 기반 프로젝트에서 직접 Aspose.BarCode for Java를 가져옵니다.
...
<ul><li>BarCodeReader 클래스의 인스턴스 생성</li><li>인터리브 2-of-5(ITF) 바코드을 포함하는 소스 이미지 경로 결정</li><li>DecodeType의 대상 바코드 유형을 두 번째 매개변수로 정의</li><li>인식 결과 반복</li><li>getCodeText 메서드를 실행하여 디코딩된 텍스트 로드</li></ul>
