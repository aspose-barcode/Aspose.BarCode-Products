...
{{% blocks/products/pf/agp/content h2="How to Read Codablock-F 바코드 Using .NET" %}}
Codablock-F 바코드을(를) 디코딩하려면 기능이 풍부하고 강력하며 사용하기 쉬운 바코드 라이브러리인 [Aspose.BarCode for .NET](https://products.aspose.com/barcode/net/) API를 사용해 보세요. C# 플랫폼용. [NuGet](https://www.nuget.org/packages/aspose.barcode)을 사용하고 **Aspose.BarCode**를 검색하여 패키지를 설치합니다. 또는 패키지 관리자 콘솔에서 다음 명령을 실행합니다.
...
{{% blocks/products/pf/agp/feature-section-col title="C#에서 Codablock-F 바코드 스캔 단계" %}}
...
<ul><li>Aspose.BarCode.BarCodeReader 클래스의 새 인스턴스를 생성합니다.</li><li>Codablock-F 바코드을 매개변수로 포함하는 소스 이미지 경로를 전달합니다.</li><li>적절한 DecodeType을 다음과 같이 전달합니다. 두 번째 매개변수</li><li>디코딩 결과 반복</li><li>CodeText 속성을 사용하여 바코드 텍스트 가져오기</li></ul>
