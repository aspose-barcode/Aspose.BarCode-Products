...
{{% blocks/products/pf/agp/content h2="How to Read GS1 DataBar 바코드 Using C++" %}}
GS1 DataBar 바코드을(를) 스캔하려면 C++ 플랫폼을 위한 풍부한 기능을 갖춘 강력하고 사용하기 쉬운 바코드 라이브러리인 [Aspose.BarCode for C++](https://products.aspose.com/barcode/cpp/) API를 사용해 보세요. . 최신 버전을 직접 다운로드할 수 있습니다. [NuGet](https://www.nuget.org/packages/aspose.barcode) 패키지 관리자를 열고 **Aspose.BarCode.Cpp**를 검색한 다음 패키지를 설치하면 됩니다. 또는 패키지 관리자 콘솔에서 다음 명령을 실행할 수 있습니다.
...
<ul><li>Aspose.BarCode.BarCodeReader 클래스의 객체 생성</li><li>GS1 DataBar 바코드을 매개변수로 포함하는 소스 이미지 경로 지정</li><li>필요한 바코드 유형을 DecodeType을 두 번째 매개변수로</li><li>인식 결과 반복</li><li>CodeText 속성을 사용하여 디코딩된 바코드 텍스트 로드</li></ul>
