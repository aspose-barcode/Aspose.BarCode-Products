...
{{% blocks/products/pf/agp/content h2="How to Generate 인터리브 2-of-5(ITF) 바코드 Using C++" %}}
인터리브 2-of-5(ITF) 바코드을(를) 생성하려면 C++ 플랫폼을 위한 풍부한 기능을 갖춘 강력하고 사용하기 쉬운 바코드 라이브러리인 [Aspose.BarCode for C++](https://products.aspose.com/barcode/cpp/) API를 설치하세요. . [NuGet](https://www.nuget.org/packages/aspose.barcode) 패키지 관리자에서 직접 최신 버전을 다운로드합니다. **Aspose.BarCode.Cpp**를 검색한 다음 패키지를 설치합니다. 또는 패키지 관리자 콘솔을 열고 다음 명령을 실행합니다.
...
<ul><li>BarcodeGenerator 클래스의 객체 생성</li><li>인터리브 2-of-5(ITF) 바코드을 매개변수로 전달</li><li>인코딩할 입력 텍스트를 두 번째 매개변수로 삽입</li><li>필요한 경우 이미지 크기 정의</li><li>BarcodeGenerator.Save 메서드를 호출하여 인터리브 2-of-5(ITF) 바코드 바코드를 다운로드합니다.</li></ul>
