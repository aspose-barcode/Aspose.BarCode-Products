...
{{% blocks/products/pf/agp/content h2="How to Read Čárový kód UPC (Universal Product Code). Using Android via Java" %}}
Vyzkoušejte Aspose.BarCode pro Android přes Javu, abyste mohli snadno detekovat a číst Čárový kód UPC (Universal Product Code). v aplikacích pro Android. Jedná se o bohaté, škálovatelné a uživatelsky přívětivé API pro čárové kódy určené pro aplikace Android. Získejte Aspose.BarCode pro Java přímo z projektu založeného na Maven přidáním následujících konfigurací do build.gradle:
...
{{% blocks/products/pf/agp/feature-section-col title="Kroky pro skenování Čárový kód UPC (Universal Product Code). v Java" %}}
...
<ul><li>Vytvořte instanci třídy BarCodeReader</li><li>Určete cestu ke zdrojovému obrázku obsahující Čárový kód UPC (Universal Product Code).</li><li>Definujte cílové typy čárových kódů v DecodeType jako druhý parametr</li><li>Procházet výsledky rozpoznávání</li><li>Načíst dekódovaný text pomocí metody getCodeText</li></ul>
