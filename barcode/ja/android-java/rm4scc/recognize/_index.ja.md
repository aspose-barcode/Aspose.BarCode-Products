...
{{% blocks/products/pf/agp/content h2="How to Read Royal Mail 4-State 顧客コード (RM4SCC) Using Android via Java" %}}
Java 経由で Aspose.BarCode for Android を試して、Android アプリケーション内の Royal Mail 4-State 顧客コード (RM4SCC) を簡単に検出して読み取ります。これは、Android アプリケーションを対象とした、機能が豊富でスケーラブルで使いやすいバーコード API です。次の構成を build.gradle に追加して、Maven ベースのプロジェクトから Aspose.BarCode for Java を直接取得します。
...
{{% blocks/products/pf/agp/feature-section-col title="Java で Royal Mail 4-State 顧客コード (RM4SCC) をスキャンする手順" %}}
...
<ul><li>BarCodeReader クラスのインスタンスを作成する</li><li>Royal Mail 4-State 顧客コード (RM4SCC) を含むソース画像パスを決定する</li><li>2 番目のパラメータとして DecodeType でターゲット バーコード タイプを定義する</li><li>認識結果をループします</li><li>getCodeText メソッドを実行してデコードされたテキストをロードします</li></ul>
