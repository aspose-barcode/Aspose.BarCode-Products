...
{{% blocks/products/pf/agp/content h2="How to Read Code 16K バーコード Using Android via Java" %}}
Java 経由で Aspose.BarCode for Android を試して、Android アプリケーション内の Code 16K バーコード を簡単に検出して読み取ります。これは、Android アプリケーションを対象とした、機能が豊富でスケーラブルで使いやすいバーコード API です。次の構成を build.gradle に追加して、Maven ベースのプロジェクトから Aspose.BarCode for Java を直接取得します。
...
<ul><li>BarCodeReader クラスのインスタンスを作成する</li><li>Code 16K バーコード を含むソース画像パスを決定する</li><li>2 番目のパラメータとして DecodeType でターゲット バーコード タイプを定義する</li><li>認識結果をループします</li><li>getCodeText メソッドを実行してデコードされたテキストをロードします</li></ul>
