...
{{% blocks/products/pf/agp/content h2="How to Generate EAN (国際商品番号) バーコード Using .NET" %}}
EAN (国際商品番号) バーコード を作成するには、機能が豊富で強力で使いやすいバーコード ライブラリである [Aspose.BarCode for .NET](https://products.aspose.com/barcode/net/) API をお試しください。 C# プラットフォーム用。 [NuGet](https://www.nuget.org/packages/aspose.barcode) パッケージ マネージャーを開き、**Aspose.BarCode** を検索してパッケージをインストールします。パッケージ マネージャー コンソールから次のコマンドを実行することもできます。
...
{{% blocks/products/pf/agp/feature-section-col title="C# で EAN (国際商品番号) バーコード を生成する手順" %}}
...
<ul><li>BarcodeGenerator クラスのインスタンスを作成する</li><li>パラメータとして EAN (国際商品番号) バーコード を渡す</li><li>2 番目のパラメータとしてエンコードするテキストを設定する</li><li>必要に応じてバーコードの寸法を指定します</li><li>BarcodeGenerator.Save メソッドを呼び出して、EAN (国際商品番号) バーコード 画像を取得します</li></ul>
